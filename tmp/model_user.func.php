...
//加密函数
function lock_url($txt,$key='cdyexlzc')
{
...
// 设置 username，从前台传过来的参数才能设置，在前台无法设置cookie
function user_name_set($name) {
	global $time, $conf;
	if(empty($name)) return;
	setcookie('username', lock_url($name), $time + 8640000, $conf['cookie_path']);
...
function user_token_clear() {
	...
	setcookie('username', '', $time - 8640000, $conf['cookie_path']);
...
//	empty($user) AND http_location(url('user-login'));
	empty($user) AND http_location('http://whrhost.tk/whr/bbs/customPage/login.php'); 
